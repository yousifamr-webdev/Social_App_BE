...
class RealtimeGateway {
    ...
    authentication = async (socket, next) => {
        try {
            const { user, verifiedToken } = await this._tokenService.checkToken(socket.handshake.auth.authorization);
    ...
    initializeIO(server) {
        ...
        io.on("connection", async (socket) => {
            ...
            socket.on("disconnect", async () => {
                await this._redisService.removeSocketId(socket.data.user._id, socket.id);
            });
        });
    }
}
